...
describe('NotFound', () => {
  ...
  it('renders a more descriptive message', () => {
    ...
    expect(wrapper.find('p').first().props().children).toBe('We are sorry but the page you are looking for does not exist.');
  });

  it('renders a direction to the dashboard', () => {
    expect(wrapper.find('p').length).toBe(2);
    expect(wrapper.find('p').last().props().children[0]).toBe('Please go back to the');
